...
with st.container(border=True):
    date_col, _, room_col = st.columns([1, 2, 1])
    with date_col:
...
#===== Room Infos ==============================================================================================
with st.container(border=True):
    st.markdown(f"<h1 style='margin-top:-30px; margin-bottom:0px; padding-bottom:0; font-size:5em; color:#4A55A2;'>{room}</h1>", unsafe_allow_html=True)
    ...
    if not room_info.empty:
        ...
        st.markdown(f"<b style='font-size:1.2em;'>{room_name}</b>", unsafe_allow_html=True)
...
with st.container(border=True):
    ...
    with col2:
        search_end = st.date_input(label="End Date", key="search_end", value=pd.to_datetime("2022-09-23 23:59:59"))
